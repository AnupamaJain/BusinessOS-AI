...
|---|---|---|
| Gateway API (webhook + agent runtime) | https://saarthione-api.vercel.app | ✅ Live (Vercel, project `business-os-gateway`) |
...
```bash
curl https://saarthione-api.vercel.app/internal/llm/health -H "x-internal-key: $INTERNAL_API_KEY"
```
...
3. In the sandbox settings, set **"When a message comes in"** to:
   `https://saarthione-api.vercel.app/webhooks/twilio` (HTTP POST).
4. Set the credentials on the gateway:
   ...
   printf 'https://saarthione-api.vercel.app/webhooks/twilio' | vercel env add TWILIO_WEBHOOK_URL production
   vercel deploy --prod --yes
   ```
...
3. Configure the webhook: URL `https://saarthione-api.vercel.app/webhook`, Verify token = the `META_VERIFY_TOKEN` value from the root `.env` (already set on Vercel). Subscribe to the `messages` field.
...
vercel deploy --prod --yes
```
Set the Razorpay dashboard webhook to `https://saarthione-api.vercel.app/webhooks/razorpay` (events: `payment_link.paid`, `payment_link.expired`, `payment_link.cancelled`). Payment links are created via `RazorpayPaymentService.createPaymentLink` and persisted to the `payments` table; the webhook flips payments to `captured` and orders to `paid` after signature verification.
